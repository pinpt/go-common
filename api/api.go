...
// these are the only Certificate Authorities we trust for our certificate
var caWeTrust = map[string]bool{
...
// these are the only domains that we trust
var domainRegex = regexp.MustCompile("\\.(pinpt|ppoint)\\.(net|com|io|co)$")
...
func Post(channel string, basepath string, apiKey string, obj interface{}) (*http.Response, error) {
	...
	u, _ := url.Parse(bu)
	u.Path = basepath
	req, _ := http.NewRequest(http.MethodGet, u.String(), strings.NewReader(json.Stringify(obj)))
	if apiKey != "" {
		req.Header.Add("x-api-key", apiKey)
	}
